...
public class KitCommandExecutor implements CommandExecutor
{
	...
	public KitCommandExecutor(UsefulCommands instance)
	{
		plugin = instance;
	}
							...
							try
							{
								for (int i = 0; i < itemList.length; i++)
								{
									if (receiver.getInventory().firstEmpty() != -1)
										receiver.getInventory().addItem(itemList[i]).get(0);
									else
										receiver.getLocation().getWorld().dropItem(receiver.getLocation(), itemList[i]);
								}
							}
							catch (Exception ex)
							{
								if (receiver.getInventory().firstEmpty() == -1)
								{
									sender.sendMessage(Message.warning(receiver.getName()
											+ " didn't have enough space in their inventory to spawn the entire kit. The extra items have been dropped on the ground"));
								...
								try
								{
									for (int i = 0; i < itemList.length; i++)
									{
										if (player.getInventory().firstEmpty() != -1)
											player.getInventory().addItem(itemList[i]).get(0);
										else
											player.getLocation().getWorld().dropItem(player.getLocation(), itemList[i]);
									}
								}
								catch (Exception ex)
								{
									if (player.getInventory().firstEmpty() == -1)
									{
										sender.sendMessage(Message
												.warning("You don't have enough space in your inventory to spawn the entire kit. The extra items have been dropped on the ground"));
									}
								}
							}
					...
					if (player.hasPermission("kits.admin"))
					{
						if (!plugin.getKitManager().exists(args[1]))
						{
							...
							if (args.length > 3)
							{
								...
								{
									plugin.getKitManager().setOverwrite(args[1], false);
									player.sendMessage(Message.info("Kit " + args[1] + " overwrite is currently "
											+ (plugin.getKitManager().getOverwrite(args[1]) ? "enabled." : "disabled.")));
								}
								if (args.length > 4)
								{
									try
					...
					if (player.hasPermission("kits.others.spawn." + arg))
					{
						...
						if (receiver != null)
						{
							...
							if (plugin.getKitManager().exists(arg))
							{
									...
									try
									{
										for (int i = 0; i < itemList.length; i++)
										{
											if (receiver.getInventory().firstEmpty() != -1)
												...
												receiver.getLocation().getWorld().dropItem(player.getLocation(), itemList[i]);
										}
									}
									catch (Exception ex)
									{
										sender.sendMessage(Message
												.warning("You dodn't have enough space in your inventory to spawn the entire kit. The extra items have been dropped on the ground"));
									}
								}
								receiver.updateInventory();
								...
								if (!receiver.hasPermission("kits.bypassdelay." + arg) && plugin.getDelay(plugin.getKitManager().getDelay(arg), 1) > 0)
									plugin.addDelayedPlayer(receiver);
	...
	/*
	 * public void addKitMetric(){ if (!plugin.getMetrics().isOptOut()){
	 * plugin.getMetrics().createGraph("Number of Kits Spawned").addPlotter(new
	 * Metrics.Plotter("Kit Spawn") {
	 * @Override public int getValue() { return 1; } }); } }
	 */

	/**
	 * Bukkit's default getPlayer method does not search the entire player's
	 * name for a substring being searched for, whereas this method does.
	 * 
	 * @param name
	 *            The player's name
